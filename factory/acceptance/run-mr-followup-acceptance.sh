...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/sdd-factory-mr-followup-acceptance.XXXXXX")"
...
if ! curl -fsS "${BASE_URL}/sessions" >/dev/null 2>&1; then
  ...
  wait "${SERVER_PID}" 2>/dev/null || true
  SERVER_PID=""
  exec "${REPO_ROOT}/.venv/bin/python" factory/acceptance/run-mr-followup-acceptance.py
fi
...
PREPARE_RESPONSE="$(curl -fsS -X POST "${BASE_URL}/sessions/prepare" -H 'content-type: application/json' -d '{"task_key":"IOS-ACCEPT-MR-001"}')"
