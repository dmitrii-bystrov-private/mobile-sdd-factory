...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/sdd-factory-delivery-acceptance.XXXXXX")"
...
if ! curl -fsS "${BASE_URL}/sessions" >/dev/null 2>&1; then
  ...
  wait "${SERVER_PID}" 2>/dev/null || true
  SERVER_PID=""
  exec "${REPO_ROOT}/.venv/bin/python" factory/acceptance/run-delivery-acceptance.py
fi

CREATE_PAYLOAD='{"task_key":"IOS-ACCEPT-DELIVERY-001","workflow_profile":"oneshot","policy":{"self_review_policy":"required","boy_scout_policy":"disabled","doc_harvest_policy":"disabled"}}'
...
PREPARE_RESPONSE="$(curl -fsS -X POST "${BASE_URL}/sessions/prepare" -H 'content-type: application/json' -d '{"task_key":"IOS-ACCEPT-DELIVERY-001"}')"
