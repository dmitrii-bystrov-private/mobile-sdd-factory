...
TMP_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/sdd-factory-followup-acceptance.XXXXXX")"
...
if ! curl -fsS "${BASE_URL}/sessions" >/dev/null 2>&1; then
  ...
  wait "${SERVER_PID}" 2>/dev/null || true
  SERVER_PID=""
  exec "${REPO_ROOT}/.venv/bin/python" factory/acceptance/run-followup-reopen-acceptance.py
fi

CREATE_PAYLOAD='{"task_key":"IOS-ACCEPT-REOPEN-001","workflow_profile":"oneshot","policy":{"self_review_policy":"required","boy_scout_policy":"disabled","doc_harvest_policy":"disabled"}}'
...
PREPARE_RESPONSE="$(curl -fsS -X POST "${BASE_URL}/sessions/prepare" -H 'content-type: application/json' -d '{"task_key":"IOS-ACCEPT-REOPEN-001"}')"
...
curl -fsS -X POST "${BASE_URL}/operator/reopen-from-qa" -H 'content-type: application/json' \
  ...
  | jq -e '.followup_event_type == "qa_reopen_requested"' >/dev/null

curl -fsS -X POST "${BASE_URL}/events" -H 'content-type: application/json' \
  -d "{\"session_id\":${SESSION_ID},\"event_type\":\"implementation_completed\",\"payload\":{\"summary\":\"qa fix done\"}}" \
  | jq -e '.followup_event_type == "verification_requested"' >/dev/null
...
jq -e '([.items[].artifact_type] | index("qa_reopen_comments")) != null' <<<"${ARTIFACTS_RESPONSE}" >/dev/null

echo "Follow-up reopen operator acceptance passed for session ${SESSION_ID}."
