...
TMP_ROOT="$(mktemp -d "${TASK_ROOT}/tmp/followup-reopen-acceptance.XXXXXX")"
...
if ! curl -fsS "${BASE_URL}/sessions" >/dev/null 2>&1; then
  ...
  wait "${SERVER_PID}" 2>/dev/null || true
  SERVER_PID=""
  exec env PYTHONPATH="$REPO_ROOT${PYTHONPATH:+:$PYTHONPATH}" "${REPO_ROOT}/.venv/bin/python" factory/acceptance/run-followup-reopen-acceptance.py
fi
