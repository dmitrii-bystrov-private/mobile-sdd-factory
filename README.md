...
- `glab`
- `acli`
- `twg` for automatic Story transition preparation
...
`snapshot.sh` uses `twg` to fill empty `Dev finish date` with today's date and empty `Story Points` before moving `To Do` Stories to `In Progress`.
