# Memory

## Android project patterns

See: `android-patterns.md`

## Skill authoring rules

- Skill trigger descriptions and all instructions inside skill files must be written in English only — no Russian phrases, even as examples.

## Notion links

- Always read Notion links via MCP tool, not WebFetch. See: `feedback-notion.md`

## Standup preferences

- Do NOT highlight or mention tasks with status "Waiting other platforms" — they require no action and are not relevant to daily standup.
