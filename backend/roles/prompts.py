...
def role_runtime_rules(role_name: str) -> str:
    if role_name == "implementer":
        return (
            ...
            "- Prefer the closest existing test file as the reference implementation for structure, setup, and expectations before introducing a new style.\n"
    ...
    if role_name == "proposal-context-worker":
        return (
            ...
            "- Extract explicit HTTP/HTTPS links and explicit local file references from the snapshot; treat external links as operator-provided context references rather than mandatory fetched inputs.\n"
...
def role_handoff_prompt(
    ...
    return (
        ...
        "- For markdown payloads containing backticks, parentheses, or paths with spaces, write the markdown to a file first and pass it with helper file flags such as `--issues-markdown-file <path>` instead of inline shell arguments.\n"
